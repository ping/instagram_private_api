...
class UploadEndpointsMixin(object):
    ...
    def post_video(self, video_data, size, duration, thumbnail_data, caption='', title=None, to_reel=False, to_tv=False, **kwargs):
        """
        Upload a video
        ...
        if (to_reel or to_tv) and not self.reel_compatible_aspect_ratio(size):
            raise ValueError('Incompatible reel aspect ratio.')
        elif self.compatible_aspect_ratio(size):
            raise ValueError('Incompatible aspect ratio.')
        ...
        elif to_tv and duration > 3600.0:
